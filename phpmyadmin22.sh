...
# Deploy phpMyAdmin di Ubuntu 22.04 (LAMP stack: Apache + MySQL + PHP)
#
# Usage:
#   sudo bash deploy-phpmyadmin.sh
#
# Download langsung dari GitHub:
#   wget https://raw.githubusercontent.com/<user>/<repo>/main/deploy-phpmyadmin.sh
#   sudo bash deploy-phpmyadmin.sh
#
# atau via git:
#   git clone https://github.com/<user>/<repo>.git
#   cd <repo> && sudo bash deploy-phpmyadmin.sh
#
...
# ====== CEK OS ======
if ! grep -qi "22.04" /etc/os-release 2>/dev/null; then
  echo -e "\e[1;33m[!] Warning: OS bukan Ubuntu 22.04, lanjut dengan risiko sendiri.\e[0m"
fi
...
# Generate root password kalau belum diset
